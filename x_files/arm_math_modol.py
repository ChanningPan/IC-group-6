...
class Arm:
    ...
    def goto(self, x, y, z):
        ...
        self.deg_pos = []
        for rad in self.rad_pos:
            self.deg_pos.append(rad*180/pi)
        print(self.rad_pos)
        print(self.deg_pos)
...
def callback(e):
    ...
    ax.set_zlabel('Z / mm')
    ax.set_zlim(0, 200)
    ax.plot(xs, ys, zs, linewidth=2.5, marker='*',
...
arm = Arm(130, 130, 120, Arm.minimum_change, 't')
...
sx = Scale(root, from_=0, to_=300, orient=HORIZONTAL,
...
sz = Scale(root, from_=-20, to_=300, orient=HORIZONTAL,
